...
from tests.fixtures.t9150_packets import LIVE_82_71, make_packet
...
async def test_entities_follow_complete_measurement() -> None:
    ...
    entry.runtime_data.device.process({1: LIVE_82_71})
    ...
    assert by_key["weight"].native_value == 85.3
    assert by_key["impedance"].native_value == 482.0
    assert by_key["heart_rate"].native_value == 72
    assert by_key["bmi"].native_value == 27.8
    assert by_key["body_fat"].native_value == 27.7
    assert by_key["body_fat_mass"].native_value == 23.6
    assert by_key["lean_body_mass"].native_value == 61.7
    assert by_key["muscle_mass"].native_value == 58.6
    assert by_key["bone_mass"].native_value == 3.1
    assert by_key["visceral_fat"].native_value == 11
    assert by_key["body_age"].native_value == 30
    assert by_key["body_type"].native_value == "obese"
    ...
    assert attributes["input_weight_kg"] == 85.3
    assert attributes["input_impedance_ohm"] == 482.0
    assert attributes["profile_height_cm"] == 175
    assert attributes["profile_age"] == 28
